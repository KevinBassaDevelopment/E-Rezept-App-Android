...
fun LazyItemScope.OrderEmptyScreen(
    connectionState: ProfileHandler.ProfileConnectionState?,
    onClickRefresh: () -> Unit
) {
    Box(
        modifier = Modifier
        ...
        when (connectionState) {
            ProfileHandler.ProfileConnectionState.LoggedOut -> {
                HomeHealthCardDisconnected(
                    onClickAction = onClickRefresh
                )
            }
            ProfileHandler.ProfileConnectionState.LoggedOutWithoutTokenBiometrics -> {
                HomeConnectedWithoutTokenBiometrics(
                    onClickAction = onClickRefresh
                )
            }
            ProfileHandler.ProfileConnectionState.LoggedOutWithoutToken -> {
